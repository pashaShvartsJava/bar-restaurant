...
async def admin_registration(data : Annotated[AdminRegistration, Form()], service : AuthenticationService = Depends(get_service_dependency)):
    ...
    if existing_admin is not None and (existing_admin.status==Status.PENDING or existing_admin.status==Status.FAILED):
        old_admin_dto = AdminRegistrationDTO(identity_id=existing_admin.id, name=data.name,
                                             surname=data.surname, phone=data.phone,
...
async def edit_status(status : Status, str_email : str, service : AuthenticationService = Depends(get_service_dependency)):
    email = TypeAdapter(EmailStr).validate_python(str_email)
    existing_identity = await service.find_by_email(email)
    await service.update_status_identity(existing_identity, status)
